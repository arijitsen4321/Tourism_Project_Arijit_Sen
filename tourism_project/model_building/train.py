...
# Drop the 'Unnamed: 0' column if it exists, as it's typically an index artifact
...
# Ensure ytrain and ytest are Series if they were loaded as single-column DataFrames
...
# All features are already numerical (original or dummy-encoded) after preprocessing in prep.py
# So, apply StandardScaler to all columns in Xtrain and Xtest
numeric_features = Xtrain.columns.tolist()

# Set the class weight to handle class imbalance
class_weight = ytrain.value_counts()[0] / ytrain.value_counts()[1]

# Define the preprocessing steps (only StandardScaler is needed now)
...
with mlflow.start_run():
    ...
    model_path = "Best_Tourism_predict_model_v1.joblib"
    ...
    api.upload_file(
        path_or_fileobj="Best_Tourism_predict_model_v1.joblib",
        path_in_repo="Best_Tourism_predict_model_v1.joblib",
        repo_id=REPO_ID,
        repo_type=repo_type,
    )
